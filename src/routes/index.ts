...
router.get('/edi', async (req, res, next) => {
  try {
    const db = req.db;
    let sys = await model.hospital(db)
    const hospname = sys[0].hospname;
    const hospcode = sys[0].hospcode;

    const purchasingOrderId = req.query.purchasingOrderId;
    const obj: any = {};
    let header: any = await model.ediHeader(db, purchasingOrderId);
    header = header[0];
    obj.token = '0e6994dd108d8b5ea1d540a0f40e2eed2494298f8ba3b7ec95fb658956332abd';
    ...
    obj.header.delivery_date = '2018-10-30'; // วันที่ส่งสินค้า (Date) 
    ...
    obj.header.email = 'email@email.com'; // Email Address
    obj.header.supplier_code = 'supp999999'; // รหัสผู้จำหน่าย/ผู้ผลิต 
    ...
    obj.header.discount_pct = '0'; // ส่วนลด % 
    ...
    obj.header.last_interfaced_date = '2018-09-13 15:00:01'; // system Value 
    obj.header.interface_id = 'AB:CD:EF:12:34:56';  // system Value 
    obj.line = [];
    let detail: any = await model.ediDetail(db, purchasingOrderId);
    detail = detail[0];
    let no = 0;
    for (const v of detail) {
      ...
      line.dist_item_code = '30011100'; // *รหัสยาของผู้จัดจำหน่าย
      ...
      line.urgent_flag = true;
      line.comment = '';
      obj.line.push(line);
    }
    const send = await model.sendEDI(obj);
    res.send(send);
  } catch (error) {
    res.send(error);
  }
